...
def eval(data, network, writer, outfile, scorefile, device):
    ...
    for data in iterate_data(data, 256):
        ...
        preds = network.decode(
            words, chars, mask=masks, leading_symbolic=conll03_data.NUM_SYMBOLIC_TAGS
        )
        writer.write(
            words.cpu().numpy(), postags, chunks, preds.cpu().numpy(), labels, lengths
        )
    writer.close()
...
def main():
    ...
    word_alphabet, char_alphabet, pos_alphabet, chunk_alphabet, ner_alphabet = (
        conll03_data.create_alphabets(
            alphabet_path,
            train_path,
    ...
    data_train = conll03_data.read_bucketed_data(
        train_path,
        word_alphabet,
        char_alphabet,
        pos_alphabet,
    ...
    data_dev = conll03_data.read_data(
        dev_path,
        word_alphabet,
        char_alphabet,
        pos_alphabet,
    ...
    data_test = conll03_data.read_data(
        test_path,
        word_alphabet,
        char_alphabet,
        pos_alphabet,
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        with torch.no_grad():
            ...
            acc, precision, recall, f1 = eval(
                data_dev, network, writer, outfile, scorefile, device
            )
            print(
            ...
            print(
                ...
                % (best_acc, best_precision, best_recall, best_f1, best_epoch, patient)
            )
            print(
                "Best test acc: %.2f%%, precision: %.2f%%, recall: %.2f%%, F1: %.2f%% (epoch: %d (%d))"
                % (test_acc, test_precision, test_recall, test_f1, best_epoch, patient)
            )
            print("=" * 100)
