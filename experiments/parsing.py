...
def eval(
    ...
    for data in iterate_data(data, batch_size):
        ...
        pred_writer.write(
            words, postags, heads_pred, types_pred, lengths, symbolic_root=True
        )
    ...
    print(
        ...
        % (
            ...
            accum_lcorr,
            accum_total,
            accum_ucorr * 100 / accum_total,
            accum_lcorr * 100 / accum_total,
            accum_ucomlpete * 100 / accum_total_inst,
            accum_lcomplete * 100 / accum_total_inst,
        )
    )
    print(
        ...
        % (
            ...
            accum_lcorr_nopunc,
            accum_total_nopunc,
            accum_ucorr_nopunc * 100 / accum_total_nopunc,
            accum_lcorr_nopunc * 100 / accum_total_nopunc,
            accum_ucomlpete_nopunc * 100 / accum_total_inst,
            accum_lcomplete_nopunc * 100 / accum_total_inst,
        )
    )
    print(
        "Root: corr: %d, total: %d, acc: %.2f%%"
        % (accum_root_corr, accum_total_root, accum_root_corr * 100 / accum_total_root)
    )
    return (
        (accum_ucorr, accum_lcorr, accum_ucomlpete, accum_lcomplete, accum_total),
        (
            accum_ucorr_nopunc,
            accum_lcorr_nopunc,
            accum_ucomlpete_nopunc,
            accum_lcomplete_nopunc,
            accum_total_nopunc,
        ),
...
def train(args):
    ...
    logger.info(
        "# of Parameters: %d" % (sum([param.numel() for param in network.parameters()]))
    )

    logger.info("Reading Data")
    if alg == "graph":
        data_train = conllx_data.read_bucketed_data(
            train_path,
            word_alphabet,
            char_alphabet,
            pos_alphabet,
        ...
        data_dev = conllx_data.read_data(
            dev_path,
            word_alphabet,
            char_alphabet,
            pos_alphabet,
        ...
        data_test = conllx_data.read_data(
            test_path,
            word_alphabet,
            char_alphabet,
            pos_alphabet,
        ...
        data_dev = conllx_stacked_data.read_data(
            dev_path,
            word_alphabet,
            char_alphabet,
            pos_alphabet,
        ...
        data_test = conllx_stacked_data.read_data(
            test_path,
            word_alphabet,
            char_alphabet,
            pos_alphabet,
    ...
    logger.info(
        "training: #training data: %d, batch: %d, unk replace: %.2f"
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for step, data in enumerate(
            ...
            if alg == "graph":
                ...
                loss_arc, loss_type = network.loss(
                    words, chars, postags, heads, types, mask=masks
                )
            else:
            ...
            if step % 100 == 0:
                ...
                log_info = (
                    "[%d/%d (%.0f%%) lr=%.6f (%d)] loss: %.4f (%.4f), arc: %.4f (%.4f), type: %.4f (%.4f)"
                    % (
                        step,
                        num_batches,
                        ...
                        curr_lr,
                        num_nans,
                        train_loss / num_insts,
                        train_loss / num_words,
                        train_arc_loss / num_insts,
                        train_arc_loss / num_words,
                        train_type_loss / num_insts,
                        train_type_loss / num_words,
                    )
                )
        ...
        with torch.no_grad():
            ...
            (
                dev_ucorr_nopunc,
                dev_lcorr_nopunc,
                dev_ucomlpete_nopunc,
                dev_lcomplete_nopunc,
                dev_total_nopunc,
            ) = dev_stats_nopunct
            ...
            if (
                ...
                < dev_ucorr_nopunc + dev_lcorr_nopunc
            ):
                ...
                (
                    test_ucorrect_nopunc,
                    test_lcorrect_nopunc,
                    test_ucomlpete_nopunc,
                    test_lcomplete_nopunc,
                    test_total_nopunc,
                ) = test_stats_nopunct
            ...
            print(
                ...
                % (
                    ...
                    best_lcorrect,
                    best_total,
                    best_ucorrect * 100 / best_total,
                    best_lcorrect * 100 / best_total,
                    best_ucomlpete * 100 / dev_total_inst,
                    best_lcomplete * 100 / dev_total_inst,
                    best_epoch,
                )
            )
            print(
                ...
                % (
                    best_ucorrect_nopunc,
                    best_lcorrect_nopunc,
                    best_total_nopunc,
                    best_ucorrect_nopunc * 100 / best_total_nopunc,
                    best_lcorrect_nopunc * 100 / best_total_nopunc,
                    best_ucomlpete_nopunc * 100 / best_total_inst,
                    best_lcomplete_nopunc * 100 / best_total_inst,
                    best_epoch,
                )
            )
            print(
                ...
                % (
                    best_root_correct,
                    best_total_root,
                    best_root_correct * 100 / best_total_root,
                    best_epoch,
                )
            )
            print("-" * 125)
            print(
                ...
                % (
                    ...
                    test_lcorrect,
                    test_total,
                    test_ucorrect * 100 / test_total,
                    test_lcorrect * 100 / test_total,
                    test_ucomlpete * 100 / test_total_inst,
                    test_lcomplete * 100 / test_total_inst,
                    best_epoch,
                )
            )
            print(
                ...
                % (
                    ...
                    test_ucomlpete_nopunc * 100 / test_total_inst,
                    test_lcomplete_nopunc * 100 / test_total_inst,
                    best_epoch,
                )
            )
            print(
...
if __name__ == "__main__":
    ...
    args_parser.add_argument(
        "--mode", choices=["train", "parse"], required=True, help="processing mode"
    )
    ...
    args_parser.add_argument(
        "--batch_size", type=int, default=16, help="Number of sentences in each batch"
    )
    args_parser.add_argument(
        "--loss_type",
    ...
    args_parser.add_argument(
        "--optim", choices=["sgd", "adam"], help="type of optimizer"
    )
    args_parser.add_argument(
    ...
    args_parser.add_argument(
        "--eps", type=float, default=1e-8, help="epsilon for adam or adamax"
    )
    args_parser.add_argument(
        "--lr_decay", type=float, default=0.999995, help="Decay rate of learning rate"
    )
    ...
    args_parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of epochs to reset optimizer (default 10)",
    )
    args_parser.add_argument(
        "--weight_decay", type=float, default=0.0, help="weight for l2 norm decay"
    )
    args_parser.add_argument(
        "--unk_replace",
        type=float,
    ...
    args_parser.add_argument(
        "--punctuation", nargs="+", type=str, help="List of punctuations"
    )
    args_parser.add_argument(
        "--beam", type=int, default=1, help="Beam size for decoding"
    )
    args_parser.add_argument(
        "--word_embedding",
    ...
    args_parser.add_argument(
        "--model_path", help="path for saving model file.", required=True
    )
