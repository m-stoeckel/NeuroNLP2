...
def get_optimizer(
    parameters, optim, learning_rate, lr_decay, amsgrad, weight_decay, warmup_steps
):
    if optim == "sgd":
        optimizer = SGD(
            parameters,
...
def eval(data, network, writer, outfile, device):
    ...
    for data in iterate_data(data, 256):
        ...
        preds = network.decode(
            words, chars, mask=masks, leading_symbolic=conllx_data.NUM_SYMBOLIC_TAGS
        )
        corr += torch.eq(preds, postags).float().mul(masks).sum().item()
        total += masks.sum().item()
        writer.write(
            words.cpu().numpy(), preds.cpu().numpy(), postags.cpu().numpy(), lengths
        )
    writer.close()
    return corr, total
...
def main():
    ...
    parser.add_argument(
        "--optim", choices=["sgd", "adam"], help="type of optimizer", required=True
    )
    parser.add_argument(
    ...
    data_train = conllx_data.read_bucketed_data(
        train_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet
    )
    num_data = sum(data_train[1])
    num_labels = pos_alphabet.size()

    data_dev = conllx_data.read_data(
        dev_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet
    )
    data_test = conllx_data.read_data(
        test_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet
    )
    ...
    def construct_word_embedding_table():
        ...
        for word, index in word_alphabet.items():
            if word in embedd_dict:
                ...
                embedding = np.random.uniform(-scale, scale, [1, embedd_dim]).astype(
                    np.float32
                )
                oov += 1
    ...
    logger.info(
        "Network: %s, num_layer=%d, hidden=%d, act=%s"
    ...
    logger.info(
        "dropout(in, out, rnn): %s(%.2f, %.2f, %s)" % (dropout, p_in, p_out, p_rnn)
    )
    print(
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        print(
            "Epoch %d (%s, lr=%.6f, lr decay=%.6f, amsgrad=%s, l2=%.1e): "
        ...
        with torch.no_grad():
            ...
            if best_corr < dev_corr:
                ...
                test_corr, test_total = eval(
                    data_test, network, writer, outfile, device
                )
                print(
                    "test corr: %d, total: %d, acc: %.2f%%"
                    % (test_corr, test_total, test_corr * 100 / test_total)
                )
                patient = 0
            else:
                patient += 1
            ...
            print(
                "Best dev  corr: %d, total: %d, acc: %.2f%% (epoch: %d (%d))"
                % (
                    best_corr,
                    best_total,
                    best_corr * 100 / best_total,
                    best_epoch,
                    patient,
                )
            )
            print(
                "Best test corr: %d, total: %d, acc: %.2f%% (epoch: %d (%d))"
                % (
                    test_corr,
                    test_total,
