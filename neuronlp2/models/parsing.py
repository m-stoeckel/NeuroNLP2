...
class DeepBiAffine(nn.Module):
    ...
    def decode_local(
        self, input_word, input_char, input_pos, mask=None, leading_symbolic=0
    ):
        ...
        diag_mask = torch.eye(
            max_len, device=out_arc.device, dtype=torch.uint8
        ).unsqueeze(0)
    ...
    def decode(self, input_word, input_char, input_pos, mask=None, leading_symbolic=0):
        ...
        type_h = (
            type_h.unsqueeze(2).expand(batch, max_len, max_len, type_space).contiguous()
        )
        type_c = (
            type_c.unsqueeze(1).expand(batch, max_len, max_len, type_space).contiguous()
        )
...
class NeuroMST(DeepBiAffine):
    ...
    def loss(self, input_word, input_char, input_pos, heads, types, mask=None):
        # output from rnn [batch, length, dim]
        arc, out_type = self._get_rnn_output(
    ...
    def decode(self, input_word, input_char, input_pos, mask=None, leading_symbolic=0):
        ...
        heads, _ = parser.decode_MST(
            energy.cpu().numpy(),
            length.cpu().numpy(),
            leading_symbolic=leading_symbolic,
            labeled=False,
        )
        types = self._decode_types(
            out_type, torch.from_numpy(heads).type_as(length), leading_symbolic
        )
...
class StackPtrNet(nn.Module):
    ...
    def _get_decoder_output(
        ...
        if self.sibling:
            ...
            output_enc_sibling = (
                output_enc.gather(
                    dim=1,
                    index=siblings.unsqueeze(2).expand(batch, length_dec, enc_dim),
                )
                * mask_sib
            )
        ...
        if self.grandPar:
            ...
            output_enc_gpar = output_enc.gather(
                dim=1, index=gpars.expand(batch, length_dec, enc_dim)
            )  # * mask_gpar
    ...
    def _transform_decoder_init_state(self, hn):
        if isinstance(hn, tuple):
            ...
            if self.decoder_layers > 1:
                cn = torch.cat(
                    [cn, cn.new_zeros(self.decoder_layers - 1, batch, hidden_size)],
                    dim=0,
                )
            # hn is tanh(cn)
            hn = torch.tanh(cn)
            ...
            if self.decoder_layers > 1:
                hn = torch.cat(
                    [hn, hn.new_zeros(self.decoder_layers - 1, batch, hidden_size)],
                    dim=0,
                )
        return hn
    ...
    def decode(
        ...
        stacked_heads = torch.zeros(
            batch, 1, num_steps + 1, device=device, dtype=torch.int64
        )
        siblings = (
            torch.zeros(batch, 1, num_steps + 1, device=device, dtype=torch.int64)
            if self.sibling
            else None
        )
        ...
        for t in range(num_steps):
            ...
            if self.sibling:
                ...
                output_enc_sibling = (
                    output_enc.gather(
                        dim=1,
                        index=curr_sibs.unsqueeze(2).expand(batch, num_hyp, enc_dim),
                    )
                    * mask_sib
                )
                src_encoding = src_encoding + output_enc_sibling
            ...
            if self.grandPar:
                output_enc_gpar = output_enc.gather(
                    dim=1, index=curr_gpars.unsqueeze(2).expand(batch, num_hyp, enc_dim)
                )
                src_encoding = src_encoding + output_enc_gpar
            ...
            # [batch, num_hyp, length]
            mask_leaf = mask_leaf * (
                mask_last.unsqueeze(1) + curr_heads.ne(0)
            ).unsqueeze(2)
            ...
            if self.sibling:
                ...
                siblings[:, :, t + 1] = torch.where(
                    mask_leaf, child_index, torch.zeros_like(child_index)
                )
